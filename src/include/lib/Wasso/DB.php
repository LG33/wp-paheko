...
class DB extends KD2_DB
{
	...
	public function connect(): void
	{
		...
		try {
			$this->db = new PDO($this->driver->url, $this->driver->user, $this->driver->password, $this->driver->options);
		...
		if ($this->driver->type == 'sqlite')
		{
			...
			foreach ($this->sqlite_functions as $name => $callback)
			{
				...
				$this->db->sqliteCreateFunction($name, $callback);
			}
	...
	public function quote(string $value, int $parameter_type = PDO::PARAM_STR): string
	{
		...
		$this->connect();
		return $this->db->quote($value, $parameter_type);
	}
